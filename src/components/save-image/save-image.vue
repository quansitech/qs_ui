<template>
  <block>
    <button v-if="showAuthorizeButton" :class="['cu-btn','bg-blue',customClass]" @click="openSetting">授权保存图片</button>
    <button
      v-else
      :class="['cu-btn','bg-blue',customClass]"
      @click="saveImageToPhone"
    >{{btnText}}</button>
  </block>
</template>
...
export default {
  ...
  methods: {
    saveImageChildFun(file) {
      let that = this;
      //#ifdef MP-WEIXIN
      uni.saveImageToPhotosAlbum({ //saveImageToPhotosAlbum不支持支付宝
        filePath: file,
        success(res) {
          uni.hideLoading();
          that.base.isIos().then(res => {
            if (res) {
              uni.showToast({
                title: "图片保存成功",
                duration: 2000
    ...
    saveImage() {
      ...
      if (that.isTempDir) {
        ...
        that.fileUrl.forEach(item => {
          promiseArr.push(that.base.getImageInfo(item));
        });
